...
def vote(request, piano_id):
    if request.method == "PUT":
        ...
        try:
            ...
            if vote is None or not isinstance(vote, int):
                return HttpResponseBadRequest({"error": "Invalid vote content"})

            # Update piano vote and save
            piano.vote_count += vote
            piano.save()
            return JsonResponse({
                "content": piano.vote_count
            })
        
        except json.JSONDecodeError:
            return HttpResponseBadRequest({"error": "Invalid JSON format"})
